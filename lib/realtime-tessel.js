...
function Messaging() {
        ...
        if (!isConnected) {
            ...
            if (ortc.sockjs != null) {
                ...
                if (userPerms && userPerms != null && !hashPerm) {
                    ...
                    if(ortc.sockjs){
                        ortc.sockjs.sendText(JSON.stringify('subscribe;' + ortc.appKey + ';' + ortc.authToken + ';' + channel + ';' + hashPerm));
                    }
                }
            }
        }
    ...
    this.unsubscribe = function (channel) {
        ...
        */
        if (!isConnected) {
            ...
            if (ortc.sockjs) {
                ortc.sockjs.sendText(JSON.stringify('unsubscribe;' + ortc.appKey + ';' + channel));
            }
        }
    };
        ...
        if (!isConnected || ortc.sockjs == null) {
            ...
            if (userPerms && userPerms != null && !hashPerm) {
                ...
                if (messageParts.length == 1) {
                    if(ortc.sockjs){
                        ortc.sockjs.sendText(JSON.stringify('send;' + ortc.appKey + ';' + ortc.authToken + ';' + channel + ';' + hashPerm + ';' + messageId + '_1-1_' + messageParts[0]));
                    }  
                }
                else {
    ...
    */
    this.sendProxy = function (applicationKey, privateKey, channel, message) {
        ...
        */
        if (!isConnected || ortc.sockjs == null) {
            ...
            if (messageParts.length == 1) {
                if(ortc.sockjs){
                    ortc.sockjs.sendText(JSON.stringify('sendproxy;' + applicationKey + ';' + privateKey + ';' + channel + ';' + messageId + '_1-1_' + messageParts[0]));
                }  
            }
            else {
                for (var j = 1; j <= messageParts.length; j++) {
    ...
    this.sendMultiPartMessage = function(message){
        process.nextTick(function(){
            if(ortc.sockjs){
                ortc.sockjs.sendText(JSON.stringify(message));
            }
        });
    }
    ...
    ***********************************************************/
    ...
    var ortcIsValidInput = function (input) {
        ...
        //var match = regexPattern.exec(input);
    ...
    */
    var disconnectSocket = function () {
        reconnectStartedAt = null;
        isConnecting = false;
        if (ortc.sockjs && ortc.sockjs != null) {
            ortc.sockjs.close();
            ortc.sockjs = null;
        }
    };
    ...
    */
    var createSocketConnection = function (connectionUrl) {
        if (ortc.sockjs == null) {
            ...
            ortc.sockjs = socket.connection(function () {
                // Update last keep alive time

                lastKeepAlive = new Date().getTime();
                if(ortc.sockjs){
                    ortc.sockjs.sendText(JSON.stringify('validate;' + ortc.appKey + ';' + ortc.authToken + ';' + (announcementSubChannel ? announcementSubChannel : '') + ';' + '' + ';' + (connectionMetadata ? connectionMetadata : '')));
                }
            ...
            // Disconnect handler
            ortc.sockjs.on('close',function (code, reason) {
                ortc.sockjs = null;
            ...
            ortc.sockjs.on('error', function (e) {

            });

            // Receive handler
            ortc.sockjs.on('text', function(m) {
                // Update last keep alive time
                lastKeepAlive = new Date().getTime();
                ...
                switch (messageType) {
                    case 'o': // open
                        break;
                    case 'a': // message
                        var data = JSON.parse(JSON.parse(m.substring(1))[0]);
                        var op = data.op;
                                ...
                                if (alreadyConnectedFirstTime) {
                                    ...
                                    for (var key in subscribedChannels) {
                                        ...
                                        if (subscribedChannels[key].subscribeOnReconnected == true && (subscribedChannels[key].isSubscribing || subscribedChannels[key].isSubscribed)) {
                                            ...
                                            if(ortc.sockjs){
                                                ortc.sockjs.sendText(JSON.stringify('subscribe;' + ortc.appKey + ';' + ortc.authToken + ';' + key + ';' + hashPerm));
                                            }
                                        }
                                        else {
                            ...
                            default:
                                ...
                                if(a.length < 2){
                                    ...
                                    if (a[0].length != 8) {
                                        ...
                                        var b = a[1].split("-");
                                        if ((b[0] && b[1]) && (!isNaN(b[0]) && !isNaN(b[1]))) { 
                                            messageId = a[0];
                                            messageCurrentPart = parseInt(b[0], 10);
                                            messageTotalPart = parseInt(b[1], 10);
